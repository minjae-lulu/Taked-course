...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in tqdm.tqdm(range(self.epochs + 1)):
            if epoch == 3: # save when epoch 3 .pth file
            ...
            if epoch % 5 == 0: # save when epoch 5z .pth file
                torch.save(self.gnet.state_dict(), "_".join(['/content/gdrive/My Drive/homeworks/dgms_ass2/code+model/model', str(epoch), '.pth'])) #Change this path
            ...
            for batch_idx, (imgs, masked_imgs, masked_parts) in enumerate(self.dataloader):
                ...
                errD_real = self.binary_cross_entropy(self.dnet(masked_parts), valid)
                errD_real.backward()
                gen_parts = self.gnet(masked_imgs)
                errD_fake = self.binary_cross_entropy(self.dnet(gen_parts.detach()), fake)
                errD_fake.backward()
                ...
                gen_parts = self.gnet(masked_imgs)
                errG = self.binary_cross_entropy(self.dnet(gen_parts), valid)
                
                ## g_loss = pixel_loss + adv_loss * lambda
                g_loss = self.p_loss(gen_parts, masked_parts) + errG * 0.001
                #g_loss = self.p_loss(gen_parts, masked_parts) + errG * 0.0001
                #g_loss = self.p_loss(gen_parts, masked_parts) + errG * 0.01
                g_loss.backward()
                self.optimizer_G.step()
                d_loss = (errD_real + errD_fake)
                #d_loss = 0.5 * (errD_real + errD_fake)
...
class Tester(object):
    ...
    def _build_model(self):
        ...
        #self.gnet.load_state_dict(torch.load('/content/gdrive/My Drive/homeworks/dgms_ass2/code+model/model_3_.pth')) #Change this path
        #self.gnet.load_state_dict(torch.load('/content/gdrive/My Drive/homeworks/dgms_ass2/code+model/model_50_.pth')) #Change this path
        self.gnet.eval()
        print('Finish build model.')
