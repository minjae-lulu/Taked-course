...
class Graph
{ // Directed graph class for assignment 4
...
public:
  ...
  public:
    ...
    const int &getAux() const
    { // returns the auxiliary data
      return vobj -> aux;
    }
    EdgeList outgoingEdgeList() const
    { // returns the outgoing edge list
      return vobj -> outgoingEdgeList;
    }
    bool operator==(const Vertex &v)
    { // checks whether two vertices point to the same vertex object
      return this->vobj == v.vobj;
    }
    bool isAdjacentTo(const Vertex &v)
    { // checks whether the current vertex has an outgoing edge to v
...
public:
  ...
  Vertex insertVertex(const string &id)
  ...
  Edge insertEdge(double weight, Vertex origin, Vertex dest)
  { // inserts a new edge
  ...
  VertexList getVertexCollection() const
  ...
  void ReadFile(string filename, Vertex &s, Vertex &t)
  {
    ...
    vector<Vertex> v(N); // make n vertex.
